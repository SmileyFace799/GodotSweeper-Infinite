...
namespace SmileyFace799.RogueSweeper.model
{
	public class Game : BoardUpdateAdapter
	{
		...
		public void MiddleClick(Position boardPosition)
		{
			if (SelectedPowerUp == null && _board.BoardView.IsSmartRevealable(boardPosition)){
				_board.With(board => {
					...
					Dictionary<Position, Square> coveredSquares = board.GetOrGenerateCoveredSquares(boardPosition, numberSquare.Type, genData);
		...
		public override void OnSquareUpdated(SquareEvent @event) {
			if (@event.Square.Opened) {
				_stats.With(stats => {
					@event.Square.Type.Switch(new() {
